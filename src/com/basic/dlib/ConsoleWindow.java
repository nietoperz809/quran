 ...
 * reading/writing to System.out and System.in; The advantage of this over
 * System.xx is that it works with JDB, the contents of the window can be saved,
 * and *major feature* the typein and typeout can be edited.
 *
 * The intended use is as a programmer's shell for messages and simple command
 * processing while developing programs. See the "main" method in this file for
...
{
    private final StreamingTextArea area;
...
//    editMenu.add(new MenuItem("Find..."));
 ...
 // Create size menu
        ...
        for (int i = 0; i < sizes.length; i++)
        {
            ...
            if (sizes[i].equals(Integer.toString(fsize)))
            {
...
// Handle component events
    ...
    {
        MenuItem item = (MenuItem)evt.target;
...
//    }
  ...
  // Handle size menu
    ...
    public StreamingTextArea getTextArea()
    {
        return area;
    }
    ...
    public PrintStream PrintStream()
    {
        if (output == null)
        {
            output = new PrintStream (area.getOutputStream());
        }
        return (output);
    }
    ...
    public DataInputStream DataInputStream()
    {
        if (input == null)
        {
            input = new DataInputStream(area.getInputStream());
        }
        return (input);
    }
