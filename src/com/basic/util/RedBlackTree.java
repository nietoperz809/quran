...
public class RedBlackTree implements Serializable
{
    ...
    /**
     * Return the color of node x, note if node x is null then the color is
    ...
    /**
     * Rotate the tree right about the node y. Rotations preserve the key
     * ordering of their children nodes.
     */
    ...
    /**
     * Simple node insertion. After insertion the red-black properties are
     * "fixed" by rotating the correct number of nodes to balance the tree. See
     * add below.
     */
    ...
    /**
     * Add a new node to the B-tree. Do the insertion, followed by balancing. If
     * the node is simply a replacement, skip the balancing section.
     */
    ...
    /**
     * Print a nice graphical representation of the tree on the PrintStream
     * passed. See printTree for the output.
     */
    ...
    /**
     * Print a graphical version of the tree (ignores the 80 column limit) on
     * terminals. This function walks the tree and outputs a graphical form with
     * the keys in place. The format is as follows:
     * <pre>
     *                O fraz
     *                |
     *          @ foo +
     ...
     * @ bletch + | | O baz | |
     * @ bar +
     * </pre> Each node is represented by '@' if it is black, and 'O' if it is
     * red, followed by its key, and its children. Nodes to the right in output
     * are lower in the tree than nodes to the left, the root node is in column
    ...
    /**
     * Remove a node from the tree and if it is black, fix up the tree so that
