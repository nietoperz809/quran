...
public class Secure
{
    ...
    public Secure(VChannels channels)
    {
        ...
        try
        {
            ...
            rc4_enc = new RC4();
            rc4_update = new RC4();
            sha1 = MessageDigest.getInstance("SHA-1"); //new SHA1();
            md5 = MessageDigest.getInstance("MD5"); //new MD5();
        }
        catch (NoSuchAlgorithmException ex)
        {
            java.util.logging.Logger.getLogger(Secure.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public byte[] sign(byte[] session_key, int length, int keylen, byte[] data, int datalength) throws CryptoException
    {
        ...
        sha1.update(session_key, 0, keylen/*length*/); //engineUpdate(session_key, 0, keylen/*length*/);
        sha1.update(pad_54, 0, 40); //engineUpdate(pad_54, 0, 40);
        sha1.update(lenhdr, 0, 4); //engineUpdate(lenhdr, 0, 4);
        sha1.update(data, 0, datalength); //engineUpdate(data, 0, datalength);
        shasig = sha1.digest(); //engineDigest();
        sha1.reset(); //engineReset();

        md5.reset(); //.engineReset();
        md5.update(session_key, 0, keylen/*length*/); //.engineUpdate(session_key, 0, keylen/*length*/);
        md5.update(pad_92, 0, 48); //.engineUpdate(pad_92, 0, 48);
        md5.update(shasig, 0, 20); //.engineUpdate(shasig, 0, 20);
        md5sig = md5.digest(); //md5.engineDigest();
        md5.reset(); //engineReset();
...
//		logger.debug("Packet dec_count="+dec_count);
    ...
    public byte[] hash48(byte[] in, byte[] salt1, byte[] salt2, int salt) throws CryptoException
    {
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            sha1.update(pad, 0, i + 1); //engineUpdate(pad, 0, i + 1);
            sha1.update(in, 0, 48); //sha1.engineUpdate(in, 0, 48);
            sha1.update(salt1, 0, 32); //sha1.engineUpdate(salt1, 0, 32);
            sha1.update(salt2, 0, 32); //sha1.engineUpdate(salt2, 0, 32);
            shasig = sha1.digest(); //.engineDigest();
            sha1.reset(); //.engineReset();

            md5.update(in, 0, 48); //engineUpdate(in, 0, 48);
            md5.update(shasig, 0, 20); // engineUpdate(shasig, 0, 20);
    ...
    {
        md5.update(in, in_position, 16); //engineUpdate(in, in_position, 16);
        md5.update(salt1, 0, 32); //md5.engineUpdate(salt1, 0, 32);
        md5.update(salt2, 0, 32); //md5.engineUpdate(salt2, 0, 32);
        return md5.digest(); //.engineDigest();
    }
    ...
    public byte[] update(byte[] key, byte[] update_key) throws CryptoException
    {
        ...
        sha1.update(pad_54, 0, 40); //engineUpdate(pad_54, 0, 40);
        sha1.update(key, 0, keylength); //engineUpdate(key, 0, keylength); // changed from 8 - rdesktop 1.2.0
        shasig = sha1.digest(); //.engineDigest();
        sha1.reset(); //.engineReset();

        md5.reset(); //engineReset();
        md5.update(update_key, 0, keylength); //engineUpdate(update_key, 0, keylength); // changed from 8 - rdesktop 1.2.0
        md5.update(pad_92, 0, 48); //engineUpdate(pad_92, 0, 48);
        md5.update(shasig, 0, 20); //.engineUpdate(shasig, 0, 20);
        thekey = md5.digest(); //.engineDigest();
        md5.reset(); //engineReset();
